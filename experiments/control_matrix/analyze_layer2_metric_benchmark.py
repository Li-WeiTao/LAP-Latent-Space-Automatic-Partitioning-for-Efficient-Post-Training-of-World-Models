...
def meta(r):
 if (r/"manifest.json").exists():
  d=json.loads((r/"manifest.json").read_text()); return np.asarray(d["cluster_fractions"]),float(d["method_metadata"]["spectral"]["eigengap_after_k"])
 d=json.loads((r/"cluster_meta.json").read_text()); return np.asarray(d["full_cluster_fractions"]),float(d["spectral"]["eigengap_after_k"])
...
def task_rows(repo,t,ks,seeds,a):
 m4=json.loads(gate(repo,t,4).read_text()); x,ids=rg.load_unique(Path(m4["cache_stats"]["cache"]),a.frameskip); data=Path(m4["data_file"])
 with h5py.File(data,"r",swmr=True) as h:
  ek="episode_idx" if "episode_idx" in h else "ep_idx"; groups=np.asarray(h[ek][ids],np.int64)
 with np.load(a.audit_dir/f"{t}.npz",allow_pickle=False) as z: ax=np.asarray(z["x"],np.float32); aids=np.asarray(z["sample_ids"],np.int64)
 ...
 for k in ks:
  labels={s:rg.load_labels(root(repo,t,k,s)/"cluster_labels.npz",ids) for s in seeds}; ays={s:labels[s][apos] for s in seeds}
  ...
  xtx=np.c_[np.ones(len(aa)),aa].T@np.c_[np.ones(len(aa)),aa]; xty=np.c_[np.ones(len(aa)),aa].T@(x[right].astype(np.float64)-x[left].astype(np.float64)); gcoef=np.linalg.solve(xtx+penalty,xty)
  ...
  fractions,eig=meta(root(repo,t,k,0)); pp,op=protos(root(repo,t,k,0)); p=np.load(pp); own=np.load(op); p/=np.maximum(np.linalg.norm(p,axis=1,keepdims=True),1e-12)
...
def benchmark(repo,raw,out):
 screen=pd.read_csv(repo/"experiments/control_matrix/assets/lewm_k4_geometry_screen/metric_screen_summary.csv",float_precision="round_trip").set_index("metric"); targets=pd.read_csv(repo/"experiments/control_matrix/assets/lewm_k4_geometry_screen/frozen_bures_gate_validation.csv",float_precision="round_trip"); l1=targets[targets.num_clusters.eq(4)].set_index("task"); l2=targets[targets.num_clusters.isin((2,3))][["task","num_clusters","global_mean_percent","regional_mean_percent","delta_regional_minus_global_pp","point_estimate_winner"]]
