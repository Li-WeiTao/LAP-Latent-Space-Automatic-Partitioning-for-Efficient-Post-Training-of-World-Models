...
CACHE_DIR="${CACHE_DIR:-${REPO_ROOT}/experiments/control_matrix/assets/gate_sensitivity_cache}"
...
ARGS=(--repo-root "${REPO_ROOT}" --staging-dir "${STAGING_DIR}" --output-dir "${OUT_DIR}" --cache-dir "${CACHE_DIR}" --promote)
if [[ "${GPU_ID}" -ge 0 ]]; then
  ARGS+=(--gpu-id "${GPU_ID}")
fi
