...
script_path <- if (length(script_arg) == 1) {
  ...
  normalizePath("plot_reacher_control_matrix.R", winslash = "/")
}
...
inputs <- data.frame(
  ...
  path = c(
    file.path(repo_root, "experiments/reacher/matrix/matrix_summary.csv"),
    file.path(repo_root, "experiments/reacher/matrix_long/matrix_summary.csv")
  ),
...
write_csv_lf(
  summary_df,
  file.path(out_dir, "reacher_control_method_summary.csv")
)
...
make_plot <- function(
  ...
  label_offset,
  stem,
  label_adjust = numeric(0)
) {
  plot_df <- summary_df[summary_df$horizon == horizon, , drop = FALSE]
  label_adjustments <- setNames(rep(0, length(method_order)), method_order)
  label_adjustments[names(label_adjust)] <- label_adjust
  plot_df$label_y <- ifelse(
    is.na(plot_df$sd_percent),
    plot_df$mean_percent + label_offset,
    plot_df$upper + label_offset
  ) + unname(label_adjustments[as.character(plot_df$method_id)])
...
make_plot(
  ...
  title = "Reacher: short-horizon task success rate",
  y_limits = c(80.5, 89.5),
  y_breaks = seq(82, 88, by = 2),
  label_offset = 0.32,
  stem = "reacher_short_horizon_main"
)

make_plot(
  horizon = "Long horizon",
  title = "Reacher: long-horizon task success rate",
  y_limits = c(71.5, 83.0),
  y_breaks = seq(72, 82, by = 2),
  label_offset = 0.35,
  stem = "reacher_long_horizon_main",
  label_adjust = c(random = -0.15, kmeans = 0.55, spectral = -0.10)
)
...
message("Wrote Reacher short- and long-horizon figures to: ", out_dir)
