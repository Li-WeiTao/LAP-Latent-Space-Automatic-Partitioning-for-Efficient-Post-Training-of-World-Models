#!/usr/bin/env bash
# Reacher Auto-LAP symlinks — branch follows this task's own gate manifest
# (global or spectral), read directly from formal/gate/partition/manifest.json.
# No passport, no pre_execution_lock, no checksum gate: this is the LAP
# method's deployment decision, not an audit step.
set -euo pipefail
...
if [[ ! -f "$GATE_MANIFEST" ]]; then
  ...
  echo "[auto-lap] run experiments/reacher/subjepa/formal/scripts/run_reacher_gate.sh gate first" >&2
  exit 1
fi
