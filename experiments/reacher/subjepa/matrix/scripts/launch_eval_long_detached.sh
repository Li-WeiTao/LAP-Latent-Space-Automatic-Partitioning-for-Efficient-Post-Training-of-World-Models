#!/usr/bin/env bash
# Detached long-horizon eval only. Created for future use only; not executed
...
echo "[reacher-eval-long] pid=$pid"
echo "[reacher-eval-long] gpu_ids=$GPU_IDS"
echo "[reacher-eval-long] log=$LOG"
echo "[reacher-eval-long] tail: tail -f $LOG"
