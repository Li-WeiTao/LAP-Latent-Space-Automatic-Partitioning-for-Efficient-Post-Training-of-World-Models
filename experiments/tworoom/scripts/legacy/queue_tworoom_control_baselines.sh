...
run_joint() {
  ...
  if GPU="${gpu}" TRAIN_SEED=42 bash "${ROOT}/scripts/ablations/run_joint_continue_tworoom_1ep.sh"; then
    GPU="${gpu}" TRAIN_SEED=42 HORIZON=long bash "${ROOT}/scripts/ablations/run_joint_continue_tworoom_eval.sh" && \
      GPU="${gpu}" TRAIN_SEED=42 HORIZON=short bash "${ROOT}/scripts/ablations/run_joint_continue_tworoom_eval.sh"
    status=$?
  else
    status=$?
  fi
...
run_random() {
  ...
  if GPU="${gpu}" RANDOM_SEED="${partition_seed}" TRAIN_SEED=42 \
    bash "${ROOT}/scripts/internal/run_random_voronoi_train_predictors_50ep.sh"; then
    GPU="${gpu}" RANDOM_SEED="${partition_seed}" TRAIN_SEED=42 HORIZON=long \
      bash "${ROOT}/scripts/internal/run_random_voronoi_eval.sh" && \
