#!/usr/bin/env bash
# Geometry rooms3 latent linear-decodability probes (episode split).
...
OUT="${ROOT}/results/geometry_latent_svm_rooms3"
LOG="${OUT}/run_full_cached.log"
mkdir -p "${OUT}"

echo "==== geometry rooms3 latent probes (episode split) started at $(date) ====" | tee "${LOG}"
