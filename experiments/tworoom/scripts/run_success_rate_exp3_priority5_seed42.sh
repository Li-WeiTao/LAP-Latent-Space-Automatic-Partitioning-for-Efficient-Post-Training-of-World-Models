#!/usr/bin/env bash
# Experiment 3: 5 region predictors with geometry priority switching per MPC replan.
set -euo pipefail
...
OUT_DIR="experiments/tworoom/results/tworoom_success_rate_priority5_seed42"
BASELINE_STARTS="experiments/tworoom/results/tworoom_success_rate_baseline_seed42/results.json"

mkdir -p "${OUT_DIR}"

echo "==== exp3 priority5 success rate (seed=42) started at $(date) ===="
