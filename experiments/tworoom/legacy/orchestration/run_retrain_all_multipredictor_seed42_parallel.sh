...
for i in 0 1 2; do
  ...
  (
    export CUDA_VISIBLE_DEVICES="${gpu}"
    OUTER_SEED="${i}" bash "${ROOT}/scripts/run_latent_kmeanspp_train_predictors_50ep.sh"
...
if [[ "${RUN_EVAL}" == "1" ]]; then
  ...
  for i in 0 1 2; do
    OUTER_SEED="${i}" bash "${ROOT}/scripts/run_success_rate_5seed_latent_kmeanspp_longrange.sh" 2>&1 | tee -a "${MASTER}"
  done
fi
