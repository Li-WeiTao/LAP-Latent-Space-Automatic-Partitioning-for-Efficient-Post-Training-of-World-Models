...
bash "${ROOT}/scripts/run_geometry_train_region_predictors.sh" 2>&1 | tee -a "${MASTER}"

log "---- Phase 1b: 50ep best (doorway, left, right) ----"
bash "${ROOT}/scripts/run_geometry_train_region_predictors_50ep_best.sh" 2>&1 | tee -a "${MASTER}"

log "---- Phase 1c: 80ep doorway best ----"
bash "${ROOT}/scripts/run_geometry_train_doorway_80ep_best.sh" 2>&1 | tee -a "${MASTER}"

log "---- Phase 1d: global FT 65ep (compute-matched) ----"
bash "${ROOT}/scripts/run_geometry_train_global_ft_65ep.sh" 2>&1 | tee -a "${MASTER}"

log "---- Phase 1e: latent kmeanspp predictor FT (outer 0,1,2) ----"
for OUTER_SEED in 0 1 2; do
  log "  outer seed ${OUTER_SEED}"
  OUTER_SEED="${OUTER_SEED}" bash "${ROOT}/scripts/run_latent_kmeanspp_train_predictors_50ep.sh" 2>&1 | tee -a "${MASTER}"
done

if [[ "${RUN_EVAL}" == "1" ]]; then
  log "---- Phase 2: success-rate eval sweeps ----"
  bash "${ROOT}/scripts/run_success_rate_5seed.sh" 2>&1 | tee -a "${MASTER}"
  bash "${ROOT}/scripts/run_success_rate_5seed_50ep_rooms3_priority5.sh" 2>&1 | tee -a "${MASTER}"
  bash "${ROOT}/scripts/run_success_rate_5seed_exp5_priority5.sh" 2>&1 | tee -a "${MASTER}"
  bash "${ROOT}/scripts/run_success_rate_5seed_exp6_longrange.sh" 2>&1 | tee -a "${MASTER}"
  bash "${ROOT}/scripts/run_success_rate_5seed_exp6_longrange_30ep_50ep.sh" 2>&1 | tee -a "${MASTER}"
  bash "${ROOT}/scripts/run_success_rate_5seed_exp8_global_ft.sh" 2>&1 | tee -a "${MASTER}"
  bash "${ROOT}/scripts/run_success_rate_5seed_exp8_global_ft_longrange.sh" 2>&1 | tee -a "${MASTER}"
  for OUTER_SEED in 0 1 2; do
    OUTER_SEED="${OUTER_SEED}" bash "${ROOT}/scripts/run_success_rate_5seed_latent_kmeanspp_longrange.sh" 2>&1 | tee -a "${MASTER}"
  done
