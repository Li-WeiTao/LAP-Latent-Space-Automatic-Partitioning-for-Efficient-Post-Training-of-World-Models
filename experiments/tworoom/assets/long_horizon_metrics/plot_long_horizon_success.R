...
# Long-horizon TwoRoom success-rate figure.
...
baseline_path <- file.path(input_dir, "tworoom_success_rate_exp6_5seed_summary.csv")
ft45_seeds <- c(0, 42, 625)
ft45_paths <- setNames(
  file.path(
    input_dir,
    sprintf(
      "tworoom_success_rate_global_ft_45ep_trainseed%s_long_5eval_summary.csv",
      ft45_seeds
    )
  ),
  ft45_seeds
...
aggregate <- aggregate[aggregate$horizon == "long", , drop = FALSE]
...
results$label_y <- results$upper + ifelse(results$model == "Baseline", 0.70, 0.35)
