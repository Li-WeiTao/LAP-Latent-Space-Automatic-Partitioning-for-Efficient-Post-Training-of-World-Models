...
input_csv <- normalizePath(
  get_arg("--input", file.path(script_dir, "tworoom_subjepa_control_summary.csv")),
  winslash = "/",
  mustWork = TRUE
)
gate_csv <- normalizePath(
  get_arg("--gate-input", file.path(script_dir, "tworoom_subjepa_gate_metrics.csv")),
  winslash = "/",
  mustWork = TRUE
)
...
gate <- read.csv(gate_csv, check.names = FALSE, stringsAsFactors = FALSE)
...
auto_scope <- unique(results$source_scope[results$method_id == "autolap"])
if (length(auto_scope) != 1L || !grepl("preset deployment seed 0", auto_scope, fixed = TRUE)) {
  stop("Auto-LAP must use Spectral K3-50 at preset deployment seed 0.")
}
if (!identical(unique(gate$selected_branch), "spectral")) {
  stop("Gate table does not select the spectral branch.")
}
if (!identical(unique(gate$deployment_seed), 0L)) {
  stop("Gate table does not use deployment seed 0.")
}
...
auto_lap_caption <- "Auto-LAP = Spectral K3-50 with preset deployment seed 0."
...
make_plot <- function(horizon) {
  ...
  label_offset <- if (horizon == "short") 0.13 else 0.24
  ...
  y_limits <- if (horizon == "short") c(93.25, 95.45) else c(55.4, 60.8)
  y_breaks <- if (horizon == "short") seq(93.5, 95.0, by = 0.5) else seq(56, 60, by = 1)
  ...
  p <- ggplot(
    ...
    labs(
      title = paste0("TwoRoom Sub-JEPA: ", horizon, "-horizon task success rate"),
      subtitle = paste0(
  ...
  stem <- paste0("tworoom_subjepa_", horizon, "_horizon_main")
  ggsave(
...
message("Wrote TwoRoom Sub-JEPA short- and long-horizon PNG/PDF figures to: ", output_dir)
